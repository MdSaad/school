 ...
 $(document).on("blur", "#stuCurrentID", function(e)
	{
		...
		$.ajax(
		{
			...
			success:function(data){
			 if(data){
				$('#update_id').val(data.stu_auto_id);
				$('#f_name').val(data.f_name);
				$('#f_pre_adrs').val(data.f_pre_adrs);
				$('#f_per_adrs').val(data.f_per_adrs);
				$('#f_occupation').val(data.f_occupation);
				$('#f_occupation_adrs').val(data.f_occupation_adrs);
				$('#f_yearly_income').val(data.f_yearly_income);
				$('#f_mobile').val(data.f_mobile);
				$('#f_email').val(data.f_email);
				$('#f_passport_no').val(data.f_passport_no);
				$('#f_nid').val(data.f_nid);
				$('#f_driving_licence').val(data.f_driving_licence);
				
				$('#m_name').val(data.m_name);
				$('#m_pre_adrs').val(data.m_pre_adrs);
				$('#m_per_adrs').val(data.m_per_adrs);
				$('#m_occupation').val(data.m_occupation);
				$('#m_occupation_adrs').val(data.m_occupation_adrs);
				$('#m_yearly_income').val(data.m_yearly_income);
				$('#m_mobile').val(data.m_mobile);
				$('#m_email').val(data.m_email);
				$('#m_nid').val(data.m_nid);
				$('#m_driving_licence').val(data.m_driving_licence);
				$('#m_passport_no').val(data.m_passport_no);
				
				$('#g_name').val(data.g_name);
				$('#g_pre_adrs').val(data.g_pre_adrs);
				$('#g_per_adrs').val(data.g_per_adrs);
				$('#g_occupation').val(data.g_occupation);
				$('#g_occupation_adrs').val(data.g_occupation_adrs);
				$('#g_mobile').val(data.g_mobile);
				$('#g_email').val(data.g_email);
				$('#g_passport_no').val(data.g_passport_no);
				$('#g_nid').val(data.g_nid);
				
				 $('.update').text("Update");
			  } else { 
			     $('#update_id').val("");
			     $("#addForm").find("input[type=text]").val("");
				  $('#stuCurrentID').val(stuCurrentID);
				 $('.update').text("Save");
			  }
			}
		});
